...
def gmail_action():
    try:
        ...
        # ── AI MULTI-SEND (different message for different recipients) ──
        elif action == 'ai_multi_send':
            instruction = data.get('instruction', '')

            agent_system = (
                "You are an Email Agent. The user will describe multiple emails to send "
                "to different recipients with different messages, all in one instruction.\n"
                "Parse the instruction carefully and identify EACH recipient email address along with "
                "what message should be sent to them.\n"
                "Return ONLY a JSON array, nothing else, no markdown, no explanation, in this exact format:\n"
                '[{"to": "email@example.com", "subject": "Subject here", "body": "Full email body here"}, ...]\n'
                "For each recipient, write a complete, professional, warm email body (3-6 sentences) "
                "based on what the user wants to tell them. Generate an appropriate subject line too.\n"
                "If the user gave a short instruction per person (e.g. 'tell them meeting is cancelled'), "
                "expand it into a full professional email.\n"
                "Write the email content in the same language the user used in their instruction "
                "(Urdu/Hinglish/English), but JSON keys (to, subject, body) must remain in English.\n"
                "If you cannot find a valid email address for someone mentioned, skip that entry."
            )

            ai_resp = client.models.generate_content(
                model="gemini-3.5-flash",
                contents=f"User instruction:\n{instruction}\n\nGenerate the JSON array now. Return ONLY the JSON array, nothing else.",
                config=types.GenerateContentConfig(
                    system_instruction=agent_system,
                    temperature=0.7,
            ...
            try:
                # Try to extract JSON array even if AI added extra text around it
                start = text.find('[')
                end = text.rfind(']') + 1
                if start != -1 and end != 0:
                    text = text[start:end]
                email_list = pyjson.loads(text)
            except Exception as parse_err:
                return jsonify({"error": f"Could not parse AI response: {str(parse_err)}", "raw": text}), 200

            if not isinstance(email_list, list) or len(email_list) == 0:
                return jsonify({"error": "No valid emails found in instruction", "raw": text}), 200

            results = []
            for item in email_list:
                to = item.get('to', '').strip()
                subject = item.get('subject', 'Update').strip()
